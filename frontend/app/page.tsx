...
const steps = [
  {
    n: "01",
    title: "Unggah",
    body: "Tarik file CSV berisi kolom teks tweet ke kotak unggah, atau pilih dari perangkatmu.",
  },
  {
    n: "02",
    title: "Diproses",
  ...
  },
  {
    n: "03",
    title: "Lihat hasil",
    body: "Ringkasan distribusi sentimen, grafik, dan tabel per-tweet siap ditinjau.",
  },
];

const csvColumns = [
  { name: "full_text", required: true, note: "isi teks tweet yang dianalisis" },
  ...
  return (
    <main>
      ...
      <header className="mx-auto flex max-w-6xl items-center justify-between px-6 py-6 sm:px-10">
        ...
        <a
          href="#unggah"
          className="rounded-full border border-ink/15 px-4 py-1.5 font-sans text-sm text-ink hover:border-ink/30"
        >
          Mulai
        </a>
      </header>
      ...
      <section
        id="unggah"
        className="mx-auto grid max-w-6xl gap-12 px-6 pb-20 pt-8 sm:px-10 sm:pb-28 sm:pt-14 lg:grid-cols-[1.1fr_1fr] lg:items-center"
      >
        <div className="motion-safe:animate-rise-in">
          ...
          <h1 className="font-display text-4xl leading-[1.08] text-ink sm:text-5xl lg:text-[3.4rem]">
            Ribuan cuitan soal{" "}
            <span className="text-turmeric-deep">Makan Bergizi Gratis.</span>{" "}
            Satu jawaban jelas.
          </h1>
          <p className="mt-6 max-w-md font-sans text-base leading-relaxed text-muted">
            Unggah data tweet dalam format CSV, dan lihat bagaimana publik
            benar-benar merespons — positif, negatif, atau netral — dalam
            hitungan detik.
          </p>
      ...
      <section
        id="cara-kerja"
        className="border-t border-ink/10 bg-paper-deep/60"
      >
        <div className="mx-auto max-w-6xl px-6 py-20 sm:px-10">
          <p className="eyebrow mb-3">Cara kerja</p>
          <h2 className="mb-12 max-w-lg font-display text-2xl text-ink sm:text-3xl">
            Tiga langkah dari file mentah ke kesimpulan.
          </h2>

          <div className="grid gap-10 sm:grid-cols-3 sm:gap-8">
            {steps.map((s) => (
              <div key={s.n}>
                <span className="font-mono text-sm text-turmeric-deep">
                  {s.n}
                </span>
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_1.3fr] lg:gap-16">
          <div>
            ...
            <p className="mt-4 font-sans text-sm leading-relaxed text-muted">
              Cukup satu kolom teks yang wajib ada. Kolom lain sifatnya
              opsional dan akan dipakai untuk memperkaya tampilan hasil.
            </p>
          </div>
                ...
                {csvColumns.map((c) => (
                  <tr key={c.name} className="border-b border-ink/5 last:border-0">
                    ...
                    <td className="px-5 py-3">
                      {c.required ? (
                        <span className="tag-chip tag-chip--negative">ya</span>
                      ) : (
                        <span className="text-muted">tidak</span>
                      )}
                    </td>
                    <td className="px-5 py-3 font-sans text-muted">
                      {c.note}
                    </td>
                  </tr>
                ))}
      ...
      <footer className="border-t border-ink/10">
        <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-4 px-6 py-10 sm:flex-row sm:items-center sm:px-10">
          ...
          <p className="font-mono text-[11px] text-muted">
            dibangun untuk analisis sentimen data tweet berbahasa Indonesia
          </p>
        </div>
      </footer>
    </main>
